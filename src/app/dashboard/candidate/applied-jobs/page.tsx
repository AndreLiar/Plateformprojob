...
import { Briefcase, FileSearch, Clock, PlusCircle, DownloadCloud, ExternalLink } from "lucide-react";
...
export default function AppliedJobsPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      setLoadingJobs(false); // Not logged in, not loading auth, so no jobs to load
    }
  }, [user, authLoading]);
                ...
                <Card key={app.id} className="shadow-md hover:shadow-lg transition-shadow rounded-md overflow-hidden">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-xl text-primary hover:underline">
                           {/* Link to the actual job detail page if it exists, otherwise just text */}
                           {/* For now, assume /jobs will show all, and no specific job ID page for candidate */}
                           {app.jobTitle || "Job Title Not Available"}
                        </CardTitle>
                        {/* Company Name could be denormalized into application if needed */}
                        {/* <CardDescription className="text-sm">{app.companyName || "Company Not Available"}</CardDescription> */}
                      </div>
                  ...
                  <CardContent className="pt-0 pb-4 text-sm text-muted-foreground space-y-1">
                   ...
                   <CardFooter className="bg-muted/30 py-3 px-6 border-t flex justify-between items-center">
                     {/* Future actions like "Withdraw Application" could go here */}
                     <Button variant="link" size="sm" className="p-0 h-auto text-primary opacity-50 cursor-not-allowed">
